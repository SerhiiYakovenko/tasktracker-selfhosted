...
The follow-up repository for the O'Reilly live course **AI Code Review in Production**. The course ends with an AI reviewer running as a GitHub Action from one workflow file and one secret; this repository is the same TaskTracker app reviewed by **a self-hosted fork of PR-Agent running as a GitHub App**, with custom review logic and slash commands. It is what you build when you outgrow the Action: your own model, your own data boundary, your own rules engine.

The `demo/add-search` branch carries the same feature pull request with intentionally planted review findings, so the two reviewers can be compared on identical code.
...
This repo's self-hosted setup runs three components:

1. **GitHub App**: registers your fork of [PR-Agent](https://github.com/the-pr-agent/pr-agent) as a GitHub App (permissions: read contents, write pull requests / issues). The app receives webhooks for PR events and slash commands via the webhook server.
...
3. **Custom `/check_standards` command**: a hand-written tool and tuned prompt in the fork that catches team conventions (hardcoded secrets, camelCase naming, bare exceptions, missing tests). Triggers on demand via `/check_standards` or automatically on every new PR, depending on your `configuration.toml` settings.

**The flow:** you push code, the PR opens, the webhook fires, the bot clones the repo, runs your configured tools (`/describe`, `/review`, `/check_standards`), and posts a single GitHub comment with findings. Each push triggers an incremental review. No polling, no external services scanning your code; it all runs on your server.

**Trade-off vs. the marketplace Action:** you own the infrastructure, control the model and cost, and can add custom rules. You also run a server (a free-tier Render instance is enough to try it, about USD 7 a month for production). The Action is instant but less flexible.
...
The `demo/add-search` branch carries a feature PR with **intentionally planted review-worthy code**:
...
These issues are not marked or commented. When the reviewer analyzes the PR, it surfaces these findings in realistic PR comments, showing exactly what a self-hosted setup can catch. The `/check_standards` command is tuned to spot conventions like hardcoded secrets and bare exceptions.

## Running the bot yourself

Start here after the course. To run the self-hosted bot on your own repo:

1. **Fork** [the-pr-agent/pr-agent](https://github.com/the-pr-agent/pr-agent).
...
2. **Build the image**: `docker build -f docker/Dockerfile --target=github_app …`.
...
- **O'Reilly live course**: *AI Code Review in Production*; the marketplace-Action repository the course builds on: https://github.com/SerhiiYakovenko/tasktracker
...
Follow-up material for the O'Reilly live course *AI Code Review in Production* (November 2026). This repository was first shown as the self-hosted half of a hands-on workshop at TechLeadConf 2026. Read the code, fork it, self-host the bot, and extend it for your team.
